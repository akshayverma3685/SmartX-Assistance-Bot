...
def download_and_upload(self, url: str, user_id: int, max_filesize_bytes: int = None):
    """
    Downloads the URL, uploads to S3, returns dict with status and url or error.
    This runs in worker. Caller can poll result or receive webhook style notification.
    """
    try:
        ...
        presigned = s3_service.generate_presigned_url(key, expires_in_seconds=24*3600)
        # cleanup
        try:
            if tmpdir:
                import shutil
        ...
        try:
            ...
            await_obj = None
            # can't await here (celery task sync) — use direct motor (blocking) or omit
            # For simplicity, write minimal to DB using motor sync? We'll skip DB log here.
        except Exception:
